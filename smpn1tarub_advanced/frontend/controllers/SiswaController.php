    ...
    public function actionIndex() {
        if (!\Yii::$app->user->isGuest) {
            if (\Yii::$app->user->identity->level_user == 3) {
                $siswa = \frontend\models\SiswakelasModel::find()->where(['nis' => Yii::$app->user->identity->username])->all();
                return $this->render('index', ['siswa' => $siswa]);
            }else{
                echo "<script> alert('Harus Login Sebagai Siswa Terlebih dahulu');location.href = '?r=site'</script>";
            }
        } else {
